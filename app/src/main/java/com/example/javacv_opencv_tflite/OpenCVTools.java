...
//                    }
     ...
     *  师兄的凸包检测代码实现
     */
    public MatNumberUtils other_contours_Hull(Mat image){
        ...
        for (MatOfPoint contour: contours) {
            ...
            if ( approx.rows() > 4 && del_area > tmp_area && tmp_area > 300 &&
                ...
                // 绘制凸包
                for (int j = 0; j < approx.rows(); j++) {
                    Imgproc.line(image, approx.toArray()[j], approx.toArray()[(j + 1) % approx.rows()], new Scalar(0,255,255), 2, 8, 0);
                }
            }
            if (number > 200){
                break;
